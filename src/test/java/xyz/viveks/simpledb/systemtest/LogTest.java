...
public class LogTest extends SimpleDbTestBase {
  ...
  void insertRow(HeapFile hf, Transaction t, int v1, int v2)
    ...
    TupleIterator insertRow =
        new TupleIterator(Utility.getTupleDesc(2), Arrays.asList(new Tuple[] {value}));
  ...
  void doInsert(HeapFile hf, int t1, int t2)
    ...
    t.start();
    if (t1 != -1) insertRow(hf, t, t1, 0);
    Database.getBufferPool().flushAllPages();
    if (t2 != -1) insertRow(hf, t, t2, 0);
    t.commit();
  }
  ...
  public void PatchTest() throws IOException, DbException, TransactionAbortedException {
    ...
    Page p =
        Database.getBufferPool()
            .getPage(t1.getId(), new HeapPageId(hf1.getId(), 0), Permissions.READ_ONLY);
