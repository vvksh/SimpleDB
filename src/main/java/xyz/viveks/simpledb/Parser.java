...
public class Parser {
  ...
  void processExpression(TransactionId tid, ZExpression wx, LogicalPlan lp)
    ...
    if (wx.getOperator().equals("AND")) {
      ...
      if (op1const && op2const) {
        isJoin =
            ((ZConstant) ops.elementAt(0)).getType() == ZConstant.COLUMNNAME
    ...
    for (int i = 0; i < selectList.size(); i++) {
      ...
      if (si.getAggregate() != null) {
        ...
        if (groupByField != null
            && !(groupByField.equals(si.getTable() + "." + si.getColumn())
  ...
  public Query handleQueryStatement(ZQuery s, TransactionId tId)
      throws TransactionAbortedException, DbException, IOException, ParsingException,
          Zql.ParseException {
    ...
    if (physicalPlan != null) {
      Class<?> c;
      try {
        c = Class.forName("xyz.viveks.simpledb.operators.OperatorCardinality");
        ...
        m.invoke(
            null, (Operator) physicalPlan, lp.getTableAliasToIdMapping(), TableStats.getStatsMap());
        c = Class.forName("xyz.viveks.simpledb.QueryPlanVisualizer");
