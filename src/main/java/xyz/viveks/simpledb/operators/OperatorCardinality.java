 ...
 *
 * <p>All methods have been fully provided. No extra codes are required.
 */
public class OperatorCardinality {
   ...
   */
  public static boolean updateOperatorCardinality(
      Operator o, Map<String, Integer> tableAliasToId, Map<String, TableStats> tableStats) {
    if (o instanceof Filter) {
    ...
    } else if (o instanceof AggregateOperator) {
      return updateAggregateCardinality((AggregateOperator) o, tableAliasToId, tableStats);
    } else {
      ...
      if (children.length > 0 && children[0] != null) {
        if (children[0] instanceof Operator) {
          ...
          childC =
              tableStats.get(((SeqScan) children[0]).getTableName()).estimateTableCardinality(1.0);
        }
      }
  ...
  private static boolean updateFilterCardinality(
    ...
    if (tableId != null) {
      selectivity =
          tableStats
              .get(Database.getCatalog().getTableName(tableId))
              .estimateSelectivity(
      ...
      if (child instanceof Operator) {
        ...
        f.setEstimatedCardinality(
            (int)
                    (tableStats.get(((SeqScan) child).getTableName()).estimateTableCardinality(1.0)
                        * selectivity)
                + 1);
        return false;
      }
  ...
  private static boolean updateJoinCardinality(
    ...
    if (child1 instanceof Operator) {
      ...
      child1Card =
          (int) (tableStats.get(((SeqScan) child1).getTableName()).estimateTableCardinality(1.0));
    }
    ...
    if (child2 instanceof Operator) {
      ...
      child2Card =
          (int) (tableStats.get(((SeqScan) child2).getTableName()).estimateTableCardinality(1.0));
    }

    j.setEstimatedCardinality(
        JoinOptimizer.estimateTableJoinCardinality(
            j.getJoinPredicate().getOperator(),
            tableAlias1,
            tableAlias2,
            pureFieldName1,
            pureFieldName2,
  ...
  private static boolean updateHashEquiJoinCardinality(
      HashEquiJoin j, Map<String, Integer> tableAliasToId, Map<String, TableStats> tableStats) {
  ...
  private static boolean updateAggregateCardinality(
      AggregateOperator a,
