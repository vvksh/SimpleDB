...
import xyz.viveks.simpledb.TupleDesc.TDItem;
import xyz.viveks.simpledb.operators.*;
...
public class QueryPlanVisualizer {
  ...
  private int calculateQueryPlanTreeDepth(OpIterator root) {
    ...
    if (!(root instanceof Operator)) return 2;
  ...
  private SubTreeDescriptor buildTree(
    ...
    if (queryPlan instanceof SeqScan) {
      ...
      if (plan instanceof Join) {
        ...
        thisNode.rightChild = right;
        thisNode.height = currentDepth;
      } else if (plan instanceof AggregateOperator) {
        AggregateOperator a = (AggregateOperator) plan;
        int upBarShift = parentUpperBarStartShift;
        String alignTxt;
        ...
        thisNode.height = currentDepth;
          ...
          .getName()
          .equals("xyz.viveks.xyz.viveks.simpledb.xyz.viveks.simpledb.Rename")) {
        String newName = null;
        int fieldIdx = 0;
        try {
  ...
  private void printTree(SubTreeDescriptor root, char[] buffer, int width) {
    ...
    if (root.height != 0) buffer[width * root.height + root.upBarPosition] = '|';
    else textHeight = root.height;
    ...
    if (root.leftChild != null && root.rightChild == null) {
      ...
      Arrays.fill(
          buffer,
          (textHeight + 1) * width + root.leftChild.upBarPosition,
          (textHeight + 1) * width + root.rightChild.upBarPosition + 1,
          '_');
